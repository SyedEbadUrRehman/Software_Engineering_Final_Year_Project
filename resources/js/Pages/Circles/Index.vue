...
const deleteCircle = (circleId) => {
    if(confirm("Are you sure you want to delete this circle?")) {
        router.delete(`/circles/${circleId}`, {
            preserveScroll: true,
        });
    }
};
...
<template>
    ...
    <MainLayout>
        <div class="max-w-[1000px] mx-auto px-4 pt-10 pb-20">
            ...
            <div class="bg-white border border-gray-200 rounded-3xl p-2 mb-12 shadow-sm transition-focus-within ring-black focus-within:ring-2">
                <div class="flex items-center gap-2">
                    ...
                    >
                        <Plus :size="24" />
                        <span class="hidden sm:inline ml-2 pr-2 font-bold">Create</span>
                    </button>
                </div>
            </div>
            ...
            <div v-if="circles.length > 0">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div
                        ...
                        :key="circle.id"
                        class="group relative bg-white border border-gray-300 rounded-[2rem] p-6 hover:shadow-xl hover:shadow-gray-100 hover:border-black transition-all duration-300 flex flex-col justify-between min-h-[180px]"
                    >
                        <div class="flex justify-between items-start">
                            <div class="bg-gray-100 p-3 rounded-2xl group-hover:bg-black group-hover:text-white transition-colors">
                                <AccountGroupOutline :size="26" />
                            </div>
                            
                            <button
                                @click="deleteCircle(circle.id)"
            ...
            >
                ...
                <h3 class="text-xl font-bold text-gray-800">No circles found</h3>
                <p class="text-gray-500 mt-1">Create your first circle to get started.</p>
            </div>
        </div>
    </MainLayout>
</template>
